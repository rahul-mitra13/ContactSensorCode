...
int main(int argc, char *argv[]) {
    ...
    // therefore, saved data can be easily analyzed (e. g. with fft)
        ...
        for (i = 0; i < samplesMaxSPI; i++) {
            //Messing with code here
            /* printf("%f\n", tDuration);
               int flag1 = 0;
               int flag2 = 0;
               if ( tDuration > 2 && tDuration < 3 && flag1 == 0){
                  system("python bringDown.py");
                  flag1 = 1;
            }
            if ( tDuration > 4 && flag2 == 0){
                system("python bringUp.py");
                 flag2 = 1;
             }*/
            //stopped messing with code here
            data[0] = DATAX0;
            bytes = readBytes(h, data, 7);
            if (bytes == 7) {
                x = (data[2]<<8)|data[1];
